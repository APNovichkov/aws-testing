...
def download_reference(s3_path, working_dir):
    """Download reference folder that has been configured to run with Spades.
...
def run_spades(reference_dir, fastq_folder_path, memory, cmd_args, working_dir):
    """Run Spades.
    ...
    # UPDATE according to Spades documentation
...
def update_sorted_reference(reference_dir):
    """Update sorted-reference.xml to map to the correct directory path.

    Since each analysis occurs in subfolder of the working directory, it will change each execution.
